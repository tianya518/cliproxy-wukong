...
// 用法:
//
//	go run ./cmd/stream-capture/ -chatgpt chatgpt.json
//	go run ./cmd/stream-capture/ -config chatgpt-cli.json -case image
//
...
func main() {
	configPath := flag.String("config", "", "单账号 CLI 配置（chatgpt-cli.json / 旧 config.json）")
	chatgptPath := flag.String("chatgpt", defaultChatGPTFile(), "ChatGPT 凭证文件（chatgpt.json；旧名 tokens.json 仍可用）")
	tokensAlias := flag.String("tokens", "", "同 -chatgpt（旧名）")
	outRoot := flag.String("out", "testdata/stream-captures", "输出根目录")
	onlyCase := flag.String("case", "", "只运行指定 case: image|txt|pdf")
	flag.Parse()

	poolPath := *chatgptPath
	if *tokensAlias != "" {
		poolPath = *tokensAlias
	}
	cliPath := *configPath
	if cliPath == "" {
		cliPath = defaultChatGPTCLIFile()
	}
	token, err := loadBearerToken(cliPath, poolPath)
	if err != nil {
...
func loadBearerToken(configPath, tokensPath string) (string, error) {
	...
	if configPath == "" {
		configPath = defaultChatGPTCLIFile()
	}
	data, err := os.ReadFile(configPath)
	if err != nil {
		return "", fmt.Errorf("读取凭证失败（-chatgpt 或 -config）: %w", err)
	}
...
func loadFromTokensJSON(path string) (string, error) {
	...
	if len(tf.Tokens) == 0 || tf.Tokens[0].AccessToken == "" {
		return "", fmt.Errorf("ChatGPT 凭证文件无 access_token")
	}
...
func defaultChatGPTFile() string {
	for _, path := range []string{"chatgpt.json", "tokens.json"} {
...
func defaultChatGPTCLIFile() string {
	for _, path := range []string{"chatgpt-cli.json", "config.json"} {
		if _, err := os.Stat(path); err == nil {
			return path
		}
	}
	return "chatgpt-cli.json"
}
