...
type ChatCompletionRequest struct {
	...
	// ProjectID 把本轮挂进 ChatGPT 项目（g-p-…）。别名 GizmoID。
	// 官网只改 conversation_mode.kind=gizmo_interaction。
	ProjectID string `json:"project_id,omitempty"`
	GizmoID   string `json:"gizmo_id,omitempty"`

	// PictureV2 为 true 时注入 system_hints picture_v2 强制走 DALL·E 生图（可与任意 model 同用，便于续接 conversation_id）
	PictureV2 bool `json:"picture_v2,omitempty"`

	// 图片生成专用参数（model 为 dall-e* 时也会自动启用 picture_v2；gpt-image* 兼容别名会映射为 dall-e-3）
	...
	// ArtifactMarkdown 控制是否在 content 末尾附加产物的 markdown 链接，未指定时默认开启。
	// 标准 OpenAI 客户端只读 choices[].message.content，不附加的话生图结果仅存在于
	// sentinel 侧信道，末端拿到的是一条空消息。自行解析侧信道的客户端可显式传 false。
...
// wantArtifactMarkdown 未显式指定 artifact_markdown 时默认开启。
func (r ChatCompletionRequest) wantArtifactMarkdown() bool {
	return r.ArtifactMarkdown == nil || *r.ArtifactMarkdown
}

func (r ChatCompletionRequest) resolvedGizmoID() string {
	if id := sentinel.NormalizeGizmoID(r.ProjectID); id != "" {
		return id
	}
	return sentinel.NormalizeGizmoID(r.GizmoID)
}
