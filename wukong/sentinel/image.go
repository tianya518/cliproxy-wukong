...
func (c *Client) ProxyImageByFileID(fileID, conversationID string, w interface{}, reqUserAgent string) error {
	...
	apiPath := fileDownloadPath(fileID, conversationID)
...
// fileDownloadPath 官网 2026-08-26 Chat 标签生图预览实抓：
// /files/download/{id}?inline=...&download_intent=...&include_library_file_state=...&conversation_id=...
func fileDownloadPath(fileID, conversationID string) string {
	return fmt.Sprintf("/backend-api/files/download/%s?conversation_id=%s&inline=false&download_intent=false&include_library_file_state=true",
		fileID, conversationID)
}

// FileDownloadURL 换取 files/download 直链（给服务端代理和实测用）。
func (c *Client) FileDownloadURL(fileID, conversationID string) (string, error) {
	return c.resolveFileDownloadURL(fileID, conversationID)
}
