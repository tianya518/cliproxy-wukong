...
type ChatResult struct {
	...
	imageGenConvPollCount       int             // conversation API 轮询次数（imagegen skill 批量路径）
	imageGenConvLastPollAt      int64           // 上次轮询时间（纳秒）
	expectedImageCount          int             // batch_requests 期望张数（0=未知）；用于多图收齐判定
	asyncTextResolved           bool            // thinking 文本轮次：探测阶段已从 conversation 取到最终正文，跳过 WS catchup
}
