...
type ChatOptions struct {
	...
	// GizmoID 非空时把本轮挂进 ChatGPT 项目（或自定义 GPT）。
	// 官网 2026-08-26 实抓：只改 conversation_mode = {kind:gizmo_interaction,gizmo_id}。
	GizmoID string
}
...
// 官网 2026-08-26 Chat 标签实抓：客户端声明可处理的本地函数与结构化回复契约。
// local.continue_in_work 只是「切到工作标签」的 UI 钩子，不会改 conversation_mode。
var chatModelResponseContracts = []map[string]interface{}{
	{
...
func (c *Client) buildConversationBody(opts ChatOptions) map[string]interface{} {
	...
	userMsgObj := map[string]interface{}{
		...
		// 2026-08-26 Chat 标签：官网只带 selected_sources + serialization_metadata。
	...
	body := map[string]interface{}{
		...
		// 官网 2026-07 起首页有「聊天 / 工作」两个标签，但 conversation_mode.kind
		// 并没有 work_mode 这个取值（前端枚举仍是 primary_assistant / gizmo_* /
		// flying_circus_customization）。「工作」是另一个产品面（TPP / work_tab），
		// 用 -wm 模型 + 独立额度，对话仍走 /f/conversation。本项目对齐聊天标签。
		// 项目内对话（2026-08-26 实抓）只把 kind 换成 gizmo_interaction 并带 gizmo_id。
		"conversation_mode":        c.conversationMode(opts),
		...
		// 我们总会先打 /f/conversation/prepare，官网成功后写 success（旧值 none）。
		"client_prepare_state":     "success",
		"local_function_names":     []string{"local.continue_in_work"},
		"model_response_contracts": chatModelResponseContracts,
	}
	if c.tempMode {
	...
	if gid := c.resolveGizmoID(opts); gid != "" {
		c.gizmoID = gid
	}
	return body
}

func (c *Client) resolveGizmoID(opts ChatOptions) string {
	if id := NormalizeGizmoID(opts.GizmoID); id != "" {
		return id
	}
	return strings.TrimSpace(c.gizmoID)
}

func (c *Client) conversationMode(opts ChatOptions) map[string]string {
	if gid := c.resolveGizmoID(opts); gid != "" {
		return map[string]string{"kind": "gizmo_interaction", "gizmo_id": gid}
	}
	return map[string]string{"kind": "primary_assistant"}
}
...
func (c *Client) ChatStream(opts ChatOptions, handler StreamHandler) (*ChatResult, error) {
	...
	body := c.buildConversationBody(opts)
