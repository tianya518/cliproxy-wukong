...
2026-08-26 Chat 标签复测（Chrome 151）：单张文生图、一次命令 4 张独立图、txt/csv/pdf/png 上传链、生成 txt+pdf 下载均仍走本文协议。
`GET /files/download/{id}` query 现为 `inline=false&download_intent=false&include_library_file_state=true&conversation_id=`（见 `fileDownloadPath`）。
4 张独立图这次 **没有** `batch_requests`：mapping 是 1 个 ghostrider 占位 + 4 个 `t2uay3k.sj1i4kz/multimodal_text`（`sediment://`），`async_status=4`，从发请求到 4 个 download 约 33 秒。
沙箱下载仍是 `GET .../interpreter/download?message_id&sandbox_path`，txt/pdf 共用同一个 message_id。
...
故 `wukong` 现有三步上传(create → PUT → `/uploaded`)无需改动,可继续使用。
...
- 旧确认端点 `/uploaded` 同样有效,`wukong` 对非图片已用 `my_files`,**无需改动**。
...
### wukong 代码已更新（`sentinel/model_resolve.go`）
