...
func TestFlareSolverrSolveUsesProxyAndFiltersCookies(t *testing.T) {
	...
	solution, err := solveFlareSolverr(context.Background(), Config{
		FlareSolverrURL: server.URL, BaseURL: "https://grok.com", ClearanceTimeout: time.Second,
	}, "socks5h://proxy:1080")
	if err != nil {
		t.Fatal(err)
	}
...
func TestFlareSolverrSolveAcceptsNoChallengeWithoutCloudflareCookies(t *testing.T) {
	...
	solution, err := solveFlareSolverr(context.Background(), Config{
		FlareSolverrURL: server.URL, ClearanceTimeout: time.Second,
	}, "")
	if err != nil {
		t.Fatal(err)
	}
...
func TestFlareSolverrSolveRejectsTunnelProxy(t *testing.T) {
	_, err := solveFlareSolverr(context.Background(), Config{}, "trojan://secret@proxy.example:443")
